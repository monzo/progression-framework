...
type SectionProps = { children: *, title: string, isHidden?: boolean }

type SectionState = { isHidden: boolean }

class SidebarSection extends React.Component<SectionProps, SectionState> {
  constructor(props: SectionProps) {
    super(props)
    this.state = {
      isHidden:
        props.isHidden && props.isHidden != null ? props.isHidden : true,
    }
  }
  toggleHidden() {
    this.setState({
      ...
      <React.Fragment>
        <SidebarPaddingSection borderedBottom border>
          <ListElement key={Math.random()}>
            <DefaultStyledLink activeClassName="active" to="/frameworks">
              <EmojiListItem>🏠</EmojiListItem>
              Home
            </DefaultStyledLink>
          </ListElement>
          <ListElement key={Math.random()}>
            <StyledLink to="/frameworks/generic">
              <EmojiListItem>📄</EmojiListItem>
              Generic
            </StyledLink>
          </ListElement>
        ...
        {frameworkElement.map(element =>
          Object.keys(element)[0] !== 'null' ? (
            <SidebarPaddingSection top key={Object.keys(element)[0]}>
              <SidebarSection title={toTitleCase(Object.keys(element)[0])}>
                <SidebarList>
                  {Object.keys(element).map(key => element[key])[0]}
                </SidebarList>
              </SidebarSection>
