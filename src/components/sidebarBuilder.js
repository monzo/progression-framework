...
const SidebarBuilder = ({
      ...
      .map(edge => {
        ...
        if (emojiRegex.exec(content.sidebarTitle)) {
          ...
          return (
            <li key={edge.node.id}>
              <strong>
                <StyledLink to={content.path} onClickFunction={toggleSidebar}>
  ...
  return (
    <React.Fragment>
      <li key={Math.random()}>
        <strong>
          <SpecialStyledLink to="/frameworks" onClickFunction={toggleSidebar}>
            <EmojiListItem>🏠</EmojiListItem>
            Home
          </SpecialStyledLink>
        </strong>
      </li>
      <SpecialListElement key={Math.random()}>
        <strong>
          <SpecialStyledLink
      ...
      {frameworkElement.map(element =>
        Object.keys(element)[0] !== 'null' ? (
          <li key={Object.keys(element)[0]}>
            <h4>{toTitleCase(Object.keys(element)[0])}</h4>
            <ul>{Object.keys(element).map(key => element[key])[0]}</ul>
          </li>
        ) : (
